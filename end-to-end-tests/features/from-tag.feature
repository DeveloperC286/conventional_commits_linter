Feature: A Git tag can be provided as an argument to indicate where to start linting from.
  ...
  Scenario Outline: You can not provide both a tag and a commit hash.
    ...
    And the argument --from-tag is provided as "<from_tag>".
    Then the linting fails.
    And the error message is either "error: The argument '--from-tag <from-tag>' cannot be used with one or more of the other specified arguments" or "error: The argument '--stdin' cannot be used with one or more of the other specified arguments".
