...
fn generate_commit_messages(
    ...
    for i in 1..2_usize.pow(number_of_variants as u32) {
        ...
        let scope_variations = variations::get_scope_variations(
            &mut linting_errors,
            is_position_in_binary_string_true(&binary_string, 1),
        );
        ...
        for commit_type in commit_type_variations {
            for scope in scope_variations {
                for description in description_variations {
                    commit_messages.push(format!("{preceding_whitespace}{commit_type}{scope}:{after_type_variation}{description}"));
                }
            }
        }
    }
